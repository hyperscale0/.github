...
and keep issues for things that are wrong.

A good bug report names the package version, shows the input, and shows what
happened next to what you expected. For the format and the language, the input
is usually a small `.udl` or `.hsx` file, and pasting it saves a round trip.

Security problems do not go here. Follow
